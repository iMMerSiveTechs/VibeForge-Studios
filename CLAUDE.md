...
<vce_engine>
  VibeForge Cognitive Engine (VCE) has a HARD BOUNDARY between UI and backend:

  Mobile UI calls ONLY:
  - engineClient.generate(message, callbacks) — from mobile/src/engine/EngineClient.ts
  - engineClient.interrupt(turnId) — cancels running turn

  Engine adapters:
  - MockEngine — simulated streaming for UI dev (no backend needed)
  - RemoteEngine — real SSE to /api/vce/chat via proxy

  State: mobile/src/lib/state/engine-store.ts (Zustand)
  Types: mobile/src/engine/types.ts (boundary types only)

  Backend orchestrator: backend/src/routes/vce.ts
  - POST /api/vce/chat — SSE streaming (route → think → stream → fuse → done)
  - POST /api/vce/interrupt/:turnId — abort upstream model fetch

  RULES:
  - UI/screens NEVER call provider APIs directly
  - No provider URLs (anthropic, openai, gemini) in mobile/src/app/
  - All API keys live on the backend, read from DB settings via selectBestProvider()
  - All AI calls go through proxy.vibecodeapp.com (backend only)
  - Deterministic router fires BEFORE any LLM spend
  - AbortController end-to-end: interrupt aborts upstream fetch (stops token burn)

  Theme: Electric Forge neon (cyan #95CBDE, magenta #A75FBB, violet #413672, bg #020203)
</vce_engine>
